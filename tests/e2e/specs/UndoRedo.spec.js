...
describe("Undo/redo", () => {
  ...
  it("Can undo position changes", () => {
    ...
    cy.get("[data-test=undo]").click({ force: true }).should("be.disabled");

    waitToRenderAllShapes();

    cy.get("[data-test=redo]").click({ force: true }).should("be.disabled");
    ...
    cy.get("[data-test=undo]").should("not.be.disabled").click({ force: true });
