...
describe("Inspector Extension Manager", () => {
  let node;
  let config;
  ...
  it("should always sort the advanced accordion to the end when the config is a container", () => {
  ...
  it("should update items that have the same id", () => {
    config.id = 1;

    registerInspectorExtension(node, config);

    const { items } = node.inspectorConfig[0];
    const [lastItem] = node.inspectorConfig[0].items.slice(-1);
    expect(items.length).toBe(2);
    expect(lastItem).toBe(config);
  });

  it("should only move advanced types to the end", () => {
    const secondConfig = {
      id: 6,
      name: "Not advanced",
      container: true
  ...
  it("should be possible to replace a field by name", () => {
    registerInspectorExtension(node, {
      container: true,
      config: {
        label: "First",
  ...
  it("should be possible to add a field to an existing accordion", () => {
    registerInspectorExtension(node, {
      id: 5,
      container: true,
      config: {
