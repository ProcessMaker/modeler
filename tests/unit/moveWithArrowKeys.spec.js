...
describe("moveWithArrowKeys", () => {
  ...
  `("should move shape $keyPress direction when the corresponding key is pressed", ({ keyPress, expected }) => {
    const shape = shapeFactory();
    const onAfterMove = jest.fn();
  ...
  it.each([["PoolLane"], ["processmaker.components.nodes.boundaryEvent.Shape"]])(
    "Does not attempt to move an invalid shape",
    (invalidShapeType) => {
