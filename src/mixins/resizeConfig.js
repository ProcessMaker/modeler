...
export default {
  ...
  methods: {
    calculateElementLimits() {
      this.elementBounds = this.poolComponent.shape
        .getEmbeddedCells()
        .filter(
          (element) =>
            element.component &&
            element.component.node.type !== laneId &&
            element.component.node.type !== "processmaker-modeler-message-flow"
        )
    ...
    configureResize() {
      ...
      this.anchorPoints = [pointBottomRight, pointBottomLeft, pointTopRight, pointTopLeft];
    ...
    fixResizeRounding() {
      ...
      this.poolComponent.shape.resize(Math.round(width), Math.round(height));
      this.poolComponent.shape.position(Math.round(x), Math.round(y));
    },
    resizeUpdate() {
    ...
    resizeTopLeft(point, newPosition, source) {
      ...
      const minHeight = sortedLanes.slice(1).reduce((height, lane) => height + lane.getBBox().height, minLaneHeight);
      ...
      const maxLaneHeight = Math.max(laneY + laneHeight - (y + this.pointHeight), minLaneHeight);
    ...
    resizeBottomLeft(point, newPosition, source) {
      ...
      const sortedLanes = this.poolComponent.sortedLanes();
      const minHeight = sortedLanes
        .slice(0, sortedLanes.length - 1)
        .reduce((height, lane) => height + lane.getBBox().height, minLaneHeight);
      ...
      const maxPoolHeight = Math.max(y - poolY, this.elementBottomY - poolY, this.poolComponent.laneSet ? minHeight : minPoolHeight);
      const maxLaneWidth = Math.max(
        laneX + laneWidth - (x + this.pointWidth),
        laneX + laneWidth - this.elementLeftX + poolPadding + labelWidth,
        minLaneWidth
