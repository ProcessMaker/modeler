    ...
    configurePoolLane() {
      ...
      if (this.node.pool) {
        if (!this.graph.getCell(this.node.pool)) {
          this.node.pool = this.graph
            .getElements()
            .find((element) => element.component && element.component.node === this.node.pool.component.node);
        }
      ...
      const pool = this.graph
        ...
        .filter((model) => model.component && model.component.node.type === "processmaker-modeler-pool")[0];
