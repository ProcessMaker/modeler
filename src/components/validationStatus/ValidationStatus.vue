  ...
  <b-card-footer class="p-0 border-0 statusbar d-flex align-items-center justify-content-end pr-3 pl-3 border-top">
    ...
    <div class="status-bar-container d-flex align-items-center justify-content-end">
      <template v-if="autoValidate">
        <button
          v-if="numberOfProblemsToDisplay === 0"
          type="button"
          data-test="validation-list-valid"
          class="btn btn-light"
          :disabled="true"
        >
          {{ $t("BPMN Valid") }}
        ...
        <button
          v-else
          type="button"
          data-test="validation-list-toggle"
          class="btn btn-light"
      ...
      <b-form-checkbox v-model="autoValidate" data-test="validation-toggle" class="h-100 d-flex align-items-center" switch>
        {{ $t("Auto validate") }}
      </b-form-checkbox>
        ...
        <div v-if="isProblemsPanelDisplayed" class="validation-container position-absolute text-left">
          <dl class="validation-container__list align-items-baseline" data-test="validation-list">
            <template v-for="(error, index) in errorList">
              <dt :key="`${error.errorId}_${index}`" class="text-capitalize">
                <font-awesome-icon
              ...
              <dd :key="`${error.errorId}_${index}_dd`">
                ...
                <p v-if="error.id" class="pl-4 mb-0">
                  <span class="font-weight-bold">{{ $t("Node ID") }}:</span> {{ error.id }}
                </p>
              </dd>
            </template>
            <template v-for="(warning, index) in warnings">
              <dt :key="warning.title + index" class="text-capitalize">
                <font-awesome-icon
                  class="status-bar-container__status-icon ml-1 mr-1 mt-1"
              ...
              <dd :key="warning.title + index + '__dd'" class="font-italic pl-4">
                {{ warning.text }}
              </dd>
            </template>
          </dl>
