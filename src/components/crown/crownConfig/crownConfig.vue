<template>
  <div v-if="showCrown" class="crown-config" :style="style" role="menu">
    ...
    <association-flow-button :node="node" :moddle="moddle" :shape="shape" v-on="$listeners" @toggle-crown-state="showCrown = $event" />
    ...
    <data-association-flow-button :node="node" :moddle="moddle" :shape="shape" v-on="$listeners" @toggle-crown-state="showCrown = $event" />

    <default-flow :node="node" v-on="$listeners" />
    ...
    <copy-button :node="node" v-on="$listeners" />

    <delete-button :graph="graph" :shape="shape" :node="node" v-on="$listeners" />
...
export default {
  ...
  computed: {
    isFlow() {
      return ["processmaker-modeler-sequence-flow", "processmaker-modeler-message-flow"].includes(this.node.type);
    },
    isTextAnnotation() {
