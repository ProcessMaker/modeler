...
export function removeBoundaryEvents(graph, node, removeNode) {
  ...
  nodeShape
    ...
    .filter((cell) => cell.component && cell.component.node.isBpmnType("bpmn:BoundaryEvent"))
...
export function getOrFindDataInput(moddle, task, sourceNode) {
  ...
  if (!dataInput) {
    task.definition.ioSpecification.dataInputs.push(
...
export function removeDataInput(task, sourceNode) {
  ...
  const dataInput = task.definition.ioSpecification.dataInputs.find((input) => input.id === dataInputId);
  if (!dataInput) {
    return;
  }
  ...
  const inputSet = task.definition.ioSpecification.inputSets[0];
  if (!inputSet) {
    return;
  }
  // Check if dataInputRef exists
  ...
  if (task.definition.ioSpecification.outputSets) {
    const outputSet = task.definition.ioSpecification.outputSets[0];
