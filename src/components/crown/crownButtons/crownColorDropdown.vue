<template>
  <div class="cog-container" role="menuitem">
    ...
    <div v-if="dropdownOpen" class="element-list color-list">
      <button
      ...
      <button type="button" class="color-button toggle-picker" @click="colorPickerOpen = !colorPickerOpen" />

      <button type="button" class="color-button p-0 fa fa-undo-alt" data-test="clear-color" @click="resetNodeStyling" />
    ...
    <b-modal ref="icon-selector-modal" title="Select a custom icon" cancel-title="Reset to Default" @cancel="resetCustomIcon">
      <div>
        <icon-selector :value="iconName" :allow-custom="false" @input="setCustomIcon" />
      </div>
    </b-modal>
  </div>
</template>
