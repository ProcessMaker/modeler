<template>
  <div>
    ...
    <b-modal ref="subprocess-modal" :title="`Previewing '${subprocessName}'`">
      <div v-if="subProcessSvg" class="text-center" v-html="subProcessSvg" />
    ...
    <b-modal ref="no-subprocess-modal" title="No subprocess selected" :hide-footer="true"> Please select a subprocess to view it.</b-modal>
  </div>
</template>
  ...
  methods: {
    clickSubprocess(shapeView, event) {
      ...
      window.ProcessMaker.apiClient
        .get(`/processes/${this.subprocessId}`, { params: { include: "svg" } })
        .then(({ data }) => {
          if (!data.svg) {
