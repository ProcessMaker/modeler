...
export default {
  ...
  methods: {
    findSourceShape() {
      ...
      const taskWithInputAssociation = this.graph
        .getElements()
        .find(
          (element) =>
            element.component &&
            element.component.node.definition.get("dataInputAssociations") &&
      ...
      return this.graph.getElements().find((element) => element.component && element.component.node.definition === dataObjectDefinition);
    },
    getTargetRef() {
      if (this.node.dataAssociationProps) {
