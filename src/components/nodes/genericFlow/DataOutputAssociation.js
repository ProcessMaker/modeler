...
export default class DataOutputAssociation extends DataAssociation {
  static isValid({ sourceShape, targetShape }) {
    ...
    return DataAssociation.isADataNode(targetNode) && DataOutputAssociation.isValidSourceNode(sourceNode);
  }
  ...
  makeFlowNode(sourceShape, targetShape, genericLink) {
    ...
    const node = new Node(dataOutputConfig.id, dataOutputAssociation, diagram);
