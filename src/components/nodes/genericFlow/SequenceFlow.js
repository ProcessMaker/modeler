...
export default class SequenceFlow extends Flow {
  static isValid({ sourceShape, targetShape, targetConfig }) {
    ...
    return (
      Flow.hasTargetType(targetShape) &&
  ...
  makeFlowNode(sourceShape, targetShape, genericLink) {
    ...
    return new Node("processmaker-modeler-sequence-flow", sequenceFlowDefinition, diagram);
  }

  static isValidSource(sourceShape, targetConfig) {
    return targetConfig.validateIncoming == null || targetConfig.validateIncoming(sourceShape.node);
  }
