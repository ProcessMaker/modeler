<template>
  <crown-config
    ...
    v-on="$listeners"
  >
    <add-lane-above-button v-b-tooltip.hover.viewport.d50 class="crown-config__icon" :title="$t('Lane Above')" @click="addLaneAbove" />
    <add-lane-below-button v-b-tooltip.hover.viewport.d50 class="crown-config__icon" :title="$t('Lane Below')" @click="addLaneBelow" />
  </crown-config>
</template>
...
export default {
  ...
  methods: {
    ...
    sortedLanes() {
      return this.shape
        .getEmbeddedCells()
        .filter(({ component }) => component && component.node.type === laneId)
        .sort(
          (shape1, shape2) =>
    ...
    moveElement(element, toPool) {
      ...
      if (this.laneSet) {
        ...
        const containingLane = this.laneSet.get("lanes").find((lane) => lane.get("flowNodeRef").includes(elementDefinition));
    ...
    async addLane() {
      ...
      if (!this.laneSet) {
        ...
        this.shape
          .getEmbeddedCells()
          .filter((element) => elementShouldHaveFlowNodeRef(element))
          .forEach((element) => {
    ...
    pushNewLane(definition = Lane.definition(this.moddle, this.$t)) {
      ...
      this.$emit("add-node", new Node(Lane.id, definition, diagram));
    ...
    positionAndSizeLane(laneElement, pool) {
      ...
      const isFirstLane = pool.getEmbeddedCells().filter((cell) => cell.component && cell.component.node.type === laneId).length === 1;
    ...
    fillLanes(resizingLane, direction, remove) {
      ...
      const lanesHeight = this.sortedLanes().reduce((sum, lane) => sum + lane.getBBox().height, 0);
      const heightDiff = remove ? resizingLane.get("size").height : poolHeight - lanesHeight;
    ...
    updateLaneChildren() {
      ...
      this.shape
        .getEmbeddedCells()
        .filter((element) => elementShouldHaveFlowNodeRef(element) && element.component.node.pool === this.shape)
        .forEach((element) => {
      ...
      this.sortedLanes().forEach((laneShape) => {
        ...
        const hasChanged = newRefs ? newRefs.length !== currentRefs.length : currentRefs.length > 0;
