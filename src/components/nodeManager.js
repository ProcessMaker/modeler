...
export function addNodeToProcess(node, targetProcess) {
  const ignoredNodes = (nodee) =>
    nodee.isBpmnType("bpmn:MessageFlow", dataOutputAssociationType, dataInputAssociationType) || nodee.isType("processmaker-modeler-pool");
  ...
  if (node.isType("processmaker-modeler-lane")) {
    targetProcess.get("laneSets")[0].get("lanes").push(node.definition);
    return;
  }
