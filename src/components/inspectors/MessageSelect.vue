  ...
  <div>
    ...
    <div class="d-flex" :class="{ invalid }">
      ...
      <div class="btn-group ml-1" role="group">
        <button type="button" class="btn btn-secondary btn-sm" data-cy="events-list" :title="$t('Configure')" @click="toggleConfigMessage">
    ...
    <div v-if="showNewMessage" class="card">
      <div class="card-body p-2">
        <form-input v-model="messageId" :label="$t('ID')" :error="getValidationErrorForNewId(messageId)" data-cy="events-add-id" />
        <form-input
          v-model="messageName"
          :label="$t('Name')"
          :error="getValidationErrorForNewName(messageName)"
          data-cy="events-add-name"
        />
      </div>
    ...
    <div v-if="showEditMessage" class="card">
      <div class="card-body p-2">
        <form-input
          v-model="messageName"
          :label="$t('Name')"
          :error="getValidationErrorForNameUpdate(messageName)"
    ...
    <div v-else-if="showConfirmDelete" class="card mb-3 bg-danger text-white">
      ...
      <div class="card-footer text-right p-2">
        ...
        <button
          v-if="!deleteMessageUsage(deleteMessage.id)"
          type="button"
...
export default {
  ...
  computed: {
    ...
    localMessages() {
      return store.getters.rootElements.filter((element) => element.$type === "bpmn:Message");
    },
    validNew() {
      return this.getValidationErrorForNewId(this.messageId) === "" && this.getValidationErrorForNewName(this.messageName) === "";
    },
    validUpdate() {
  ...
  methods: {
          ...
          node.flowElements.forEach((element) => {
            if (element.eventDefinitions) {
              element.eventDefinitions.forEach((event) => {
                if (event.$type === "bpmn:MessageEventDefinition" && event.messageRef && event.messageRef.id === messageId) {
                  usage.push(element);
                }
              });
            }
    ...
    getValidationErrorForNewName(name) {
      ...
      const exists = store.getters.rootElements.find((element) => element.$type === "bpmn:Message" && element.name === name);
      if (exists) {
    ...
    getValidationErrorForNameUpdate(name) {
      ...
      const exists = store.getters.rootElements.find(
        (element) => element.$type === "bpmn:Message" && element.name === name && element.id !== this.messageId
      );
      if (exists) {
        return this.$t("Message Name is duplicated");
      }
