<template>
  <div>
    ...
    <div class="d-flex" :class="{ invalid }">
      ...
      <div v-if="canEdit" class="btn-group ml-1" role="group">
        <button type="button" class="btn btn-secondary btn-sm" data-cy="events-list" :title="$t('Configure')" @click="toggleConfigSignal">
          <i class="fa fa-ellipsis-h" />
        </button>
      </div>
    </div>
    <div v-if="!can('view-signals')" class="invalid-feedback d-block">
      <div>
    ...
    <div v-if="showNewSignal" class="card">
      <div class="card-body p-2">
        <form-input v-model="signalId" :label="$t('ID')" :error="getValidationErrorForNewId(signalId)" data-cy="events-add-id" />
        <form-input v-model="signalName" :label="$t('Name')" :error="getValidationErrorForNewName(signalName)" data-cy="events-add-name" />
      </div>
      <div class="card-footer text-right p-2">
        <button
          type="button"
          class="btn-special-assignment-action btn-special-assignment-close btn btn-outline-secondary btn-sm"
          data-cy="events-cancel"
          @click="cancelAddSignal"
        >
          {{ $t("Cancel") }}
        </button>
        <button
    ...
    <div v-if="showEditSignal" class="card">
      <div class="card-body p-2">
        <form-input
          v-model="signalName"
          :label="$t('Name')"
          :error="getValidationErrorForNameUpdate(signalName)"
          data-cy="events-edit-name"
        />
      </div>
      <div class="card-footer text-right p-2">
        <button
    ...
    <div v-else-if="showConfirmDelete" class="card mb-3 bg-danger text-white">
      ...
      <div class="card-footer text-right p-2">
        <button type="button" class="btn btn-sm btn-light mr-2 p-1 font-xs" data-cy="events-cancel" @click="showConfirmDelete = false">
          {{ $t("Cancel") }}
        </button>
        <button
          v-if="!deleteSignalUsage(deleteSignal.id)"
          type="button"
          class="btn btn-sm btn-danger p-1 font-xs"
          data-cy="events-delete"
          @click="confirmDeleteSignal"
        >
          {{ $t("Save") }}
        </button>
      </div>
...
export default {
  ...
  computed: {
    ...
    localSignals() {
      return store.getters.rootElements.filter((element) => element.$type === "bpmn:Signal");
    },
    validNew() {
      return this.getValidationErrorForNewId(this.signalId) === "" && this.getValidationErrorForNewName(this.signalName) === "";
    },
    validUpdate() {
  ...
  methods: {
          ...
          node.flowElements.forEach((element) => {
            if (element.eventDefinitions) {
              element.eventDefinitions.forEach((event) => {
                if (event.$type === "bpmn:SignalEventDefinition" && event.signalRef && event.signalRef.id === signalId) {
                  usage.push(element);
                }
              });
            }
    ...
    getValidationErrorForNewId(id) {
      ...
      const exists = store.getters.rootElements.find((element) => element.id === id);
      if (exists) {
    ...
    getValidationErrorForNewName(name) {
      ...
      const exists = store.getters.rootElements.find((element) => element.$type === "bpmn:Signal" && element.name === name);
      if (exists) {
    ...
    getValidationErrorForNameUpdate(name) {
      ...
      const exists = store.getters.rootElements.find(
        (element) => element.$type === "bpmn:Signal" && element.name === name && element.id !== this.signalId
      );
      if (exists) {
        return this.$t("Signal Name is duplicated");
      }
    ...
    loadOptions(filter) {
      ...
      const { pmql } = this;
      window.window.ProcessMaker.apiClient.get(this.api, { params: { filter, pmql } }).then((response) => {
