<template>
  <span data-test="body-container">
    <tool-bar
      ...
      :panels-compressed="panelsCompressed"
      @load-xml="loadXML"
    ...
    <b-row class="modeler h-100">
      <b-tooltip v-if="tooltipTarget" :key="tooltipTarget.id" :target="getTooltipTarget" :title="tooltipTitle" />
      ...
      <mini-paper :is-open="miniMapOpen" :paper-manager="paperManager" :graph="graph" :class="{ expanded: panelsCompressed }" />
...
export default {
  ...
  watch: {
    isRendering() {
      ...
      window.ProcessMaker.navbar.alerts = window.ProcessMaker.navbar.alerts.filter((alert) => alert.alertText !== loadingMessage);
  ...
  methods: {
    ...
    highlightNode(node, event) {
      ...
      const isSameHighlightedNode = _.isEqual(node.id, this.highlightedNode.id);
    ...
    registerNode(nodeType, customParser) {
      ...
      const types = Array.isArray(nodeType.bpmnType) ? nodeType.bpmnType : [nodeType.bpmnType];
    ...
    loadDataAssociations(flowElements) {
      const tasksThatHaveDataOutputAssociations = flowElements.filter(
        (task) => task.get("dataOutputAssociations") && task.get("dataOutputAssociations").length > 0
      );
      ...
      const tasksThatHaveDataInputAssociations = flowElements.filter(
        (task) => task.get("dataInputAssociations") && task.get("dataInputAssociations").length > 0
      );
    ...
    removeUnsupportedElementAttributes(definition) {
      ...
      unsupportedElements.filter((name) => definition.get(name)).forEach((name) => definition.set(name, undefined));
    },
    ...
    replaceDefinition(definition, boundaryEvent, process) {
      ...
      const boundaryEventDiagram = this.planeElements.find((diagram) => diagram.bpmnElement === definition);
    ...
    async handleDrop({ clientX, clientY, control, nodeThatWillBeReplaced }) {
      ...
      const nodeMigrator = new NodeMigrator(nodeThatWillBeReplaced, definition, this.graph, newNode, this.processes, this.collaboration);
    ...
    removeNodesFromPool(node) {
      if (node.type === "processmaker-modeler-pool" && node.definition.processRef) {
        ...
        const { laneSets } = node.definition.processRef;
    ...
    keydownListener(event) {
      ...
      moveShapeByKeypress(event.key, store.getters.highlightedShapes, this.pushToUndoStack);
    },
